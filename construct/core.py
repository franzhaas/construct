...
class CodeGen:
    ...
    def getCachedStruct(self, fmtstr):
        fmtstr = repr(fmtstr)
        try:
            return self._structs[fmtstr]
        except KeyError:
            fname = f"formatfield_{self.allocateId()}"
            self.append(f"{fname} = struct.Struct({fmtstr})")
            self._structs[fmtstr] = fname
            return fname
...
class FormatField(Construct):
    ...
    def _emitparse(self, code):
        if self.fmtstr in {"B", "<B", ">B"}:
            ...
            return f"{code.getCachedStruct(self.fmtstr)}.unpack(io.read({self.length}))[0]"

    def _emitbuild(self, code):
        return f"(io.write({code.getCachedStruct(self.fmtstr)}.pack(obj)), obj)[1]"
    ...
    def _emitparse_optional(self, block, code, name_of_parsed_item):
        if name_of_parsed_item:
            if self.fmtstr in {"B", "<B", ">B"}:
                ...
                assignment = f"({name_of_parsed_item},) = {code.getCachedStruct(self.fmtstr)}.unpack(readBuf)"
        else:
            assignment = "pass"
        block += f"""
